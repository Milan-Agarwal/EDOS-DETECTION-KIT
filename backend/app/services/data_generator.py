...
class DataGenerator:
    ...
    def __init__(self):
        ...
        self.threat_locations = [
            {"lat": 40.7128, "lng": -74.0060, "name": "New York", "country": "US", "city": "New York"},
            {"lat": 55.7558, "lng": 37.6176, "name": "Moscow", "country": "RU", "city": "Moscow"},
            {"lat": 39.9042, "lng": 116.4074, "name": "Beijing", "country": "CN", "city": "Beijing"},
            {"lat": 51.5074, "lng": -0.1278, "name": "London", "country": "GB", "city": "London"},
            {"lat": 35.6762, "lng": 139.6503, "name": "Tokyo", "country": "JP", "city": "Tokyo"},
            {"lat": 37.7749, "lng": -122.4194, "name": "San Francisco", "country": "US", "city": "San Francisco"},
            {"lat": 52.5200, "lng": 13.4050, "name": "Berlin", "country": "DE", "city": "Berlin"},
            {"lat": -33.8688, "lng": 151.2093, "name": "Sydney", "country": "AU", "city": "Sydney"},
            {"lat": 48.8566, "lng": 2.3522, "name": "Paris", "country": "FR", "city": "Paris"},
            {"lat": 43.6532, "lng": -79.3832, "name": "Toronto", "country": "CA", "city": "Toronto"},
            {"lat": 25.2048, "lng": 55.2708, "name": "Dubai", "country": "AE", "city": "Dubai"},
            {"lat": 1.3521, "lng": 103.8198, "name": "Singapore", "country": "SG", "city": "Singapore"},
            {"lat": 19.0760, "lng": 72.8777, "name": "Mumbai", "country": "IN", "city": "Mumbai"},
            {"lat": -23.5505, "lng": -46.6333, "name": "São Paulo", "country": "BR", "city": "São Paulo"},
            {"lat": 37.5665, "lng": 126.9780, "name": "Seoul", "country": "KR", "city": "Seoul"},
        ]
        ...
        self.resource_names = [
            ...
            "api-gateway", "cache-cluster", "load-balancer", "auth-server",
            "monitoring-01", "backup-server", "vpn-gateway", "firewall-01"
        ]
        
        self.resource_types = ["EC2", "RDS", "Lambda", "ElastiCache", "ELB", "VPN"]
        self.operating_systems = ["ubuntu", "centos", "windows", "mysql", "redis", "nodejs"]
    ...
    def generate_alert(self) -> Alert:
        ...
        dest_loc = random.choice([loc for loc in self.threat_locations if loc != source_loc])
        ...
        if is_attack:
            messages = [
                ...
                f"High-severity {attack['type']} detected targeting {dest_loc['name']}",
            ]
            level = attack['severity']
        else:
            messages = [
                f"Normal traffic pattern from {source_loc['name']}",
                f"Legitimate connection from {source_loc['country']}",
                f"Regular monitoring data from {source_loc['name']}",
                f"Secure connection established with {dest_loc['name']}",
            ]
            level = "LOW"
        ...
        return Alert(
            ...
            model=ModelInfo(
                ...
                probability=random.uniform(0.85, 0.99) if is_attack else random.uniform(0.1, 0.4),
                threshold=0.85
            ),
            message=random.choice(messages),
            recommendation="Block source IP immediately" if is_attack else "Continue monitoring",
            severity_score=random.randint(80, 100) if is_attack else random.randint(10, 40),
    ...
    def generate_network_traffic(self) -> Dict[str, Any]:
        ...
        for _ in range(random.randint(1, 3)):
            ...
            dest = random.choice([loc for loc in self.threat_locations if loc != source])
            ...
            arc = NetworkArc(
                ...
                attack_type=random.choice(self.attack_types)["type"] if is_attack else None,
    ...
    def generate_metrics(self) -> Dict[str, Any]:
        ...
        self.current_metrics["cpu_usage"] = max(20, min(95, 
            self.current_metrics["cpu_usage"] + random.uniform(-5, 5)))
        self.current_metrics["memory_usage"] = max(30, min(90,
            self.current_metrics["memory_usage"] + random.uniform(-3, 3)))
        self.current_metrics["network_io"] = max(50, min(300,
        ...
        return MetricsResponse(
            ...
            network=NetworkMetrics(
                total_connections=max(10000, self.current_metrics["active_connections"]),
    ...
    def generate_cloud_resources(self, count: int = 12) -> List[CloudResource]:
        ...
        for i in range(count):
            resource = CloudResource(
                ...
                status=ResourceStatus(random.choice(["running", "running", "running", "stopped"])), # Mostly running
                health=ResourceHealth(random.choice(["healthy", "healthy", "warning", "critical"])), # Mostly healthy
                cpu=random.uniform(10, 85),
                memory=random.uniform(20, 90),
                disk=random.uniform(30, 80),
                region=random.choice(["us-east-1", "us-west-2", "eu-central-1", "ap-south-1", "eu-west-1"]),
